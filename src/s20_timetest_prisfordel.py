...
class LinearRegression:
    """Vektet OLS med konstant (erstatter sklearn, som ikke er installert). Samme resultat som sklearn."""
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        sw = np.sqrt(w)
        self.coef_, *_ = np.linalg.lstsq(X * sw[:, None], np.asarray(y, float) * sw, rcond=None)
        return self

    def predict(self, X):
        return np.column_stack([np.ones(len(X)), np.asarray(X, float)]) @ self.coef_


"""Steg 20: timebasert test av prismekanismen (Codex' design, kjørt på fulle data).

Kopi av verifisering/codex_ekstern/analyze_norgespris_hourly.py med tre endringer:
  1. Elhub-data leses fra det fullstendige statusuttrekket i data/raw/ (norske kolonnenavn, `;`, desimalkomma).
  2. Spotpris leses fra data/raw/spot_hourly.parquet (hvakosterstrommen.no, samme kilde som Codex).
  3. Temperatur fra Open-Meteo lagres i data/raw/openmeteo_hourly_temp_pa.csv.
Terskler for strømstøtte (eks. mva) i STROMSTOTTE_TERSKEL, kilde i kommentar. Modell og bootstrap er uendret.
Utdata: output/tab_timetest_resultater.json, output/tab_timetest_prisfordel_bin.csv, output/fig_timetest_dose_respons.png.
"""
from config import RAW, OUT

ELHUB = RAW / "consumption_per_norgespris_order_status_eac_group_mba_hour_20231001_20260430.csv"
SPOT = RAW / "spot_hourly.parquet"
WEATHER = RAW / "openmeteo_hourly_temp_pa.csv"
RESULTS = OUT / "tab_timetest_resultater.json"
BIN_TABLE = OUT / "tab_timetest_prisfordel_bin.csv"
FIGURE = OUT / "fig_timetest_dose_respons.png"

EARLY = "Bestilt tidlig"
CONTROL = "Ikke bestilt"
# Terskel for strømstøtte, kr/kWh eks. mva, per kalenderår. Staten dekker 90 prosent over terskel, time for time.
# Kilde: regjeringen.no «Regjeringens strømtiltak» (id2900232) og nve.no «Dette er strømstøtteordningen».
STROMSTOTTE_TERSKEL = {2023: 0.70, 2024: 0.73, 2025: 0.75, 2026: 0.77}
NORGESPRIS = 0.40  # kr/kWh eks. mva
...
def download_prices() -> pd.DataFrame:
    ...
    s = s[s["area"].isin(AREAS)]
    return pd.DataFrame(
        {
            "timestamp_utc": pd.to_datetime(s["time_start"], utc=True),
            "price_area": s["area"].to_numpy(),
            "spot_nok_kwh": s["NOK_per_kWh"].astype(float).to_numpy(),
        }
    )
...
def make_hourly_gap() -> pd.DataFrame:
    ...
    df = pd.read_csv(ELHUB, sep=";", decimal=",", encoding="utf-8")
    df.columns = ["START_TIME", "END_TIME", "PRICE_AREA", "ESTIMATED_ANNUAL_CONSUMPTION_GROUP",
                  "NORGESPRIS_ORDER_STATUS", "CONSUMPTION", "METERINGPOINT_COUNT"]
    df = df[cols]
...
def add_prices_and_weather(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    threshold = out["date"].dt.year.map(STROMSTOTTE_TERSKEL).to_numpy(float)
    spot = out["spot_nok_kwh"]
    out["standard_support_price"] = np.where(
        spot > threshold, threshold + 0.10 * (spot - threshold), spot
    )
    out["wedge_nok_kwh"] = out["standard_support_price"] - NORGESPRIS
